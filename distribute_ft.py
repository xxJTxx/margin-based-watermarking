    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            if not water_relu and not train_relu:
                ...
                print(f"{len(water_relu)} layers are store in the lists...")    
            ...
            # Just for debug
            if batch_idx == 0:
                    print("first batch neuron loss: ", new_loss.item())    
            
            # Combine both losses
            ((1-new_loss_r)*ce_loss - (new_loss_r)*new_loss).backward()
            optimizer.step()
        ...
        print(f"Epoch {epoch+1}/{num_epochs}, Neuron Loss: {epoch_n_loss:.4f}")
...
""" def start_train(train_model, fix_model, train_loader, val_loader, optimizer, device, water_relu, train_relu, num_epochs=10, new_loss_r=0):
    ...
    for epoch in range(num_epochs):
        ...
        train_model.train()
        fix_model.train() # Not sure if it's ok to use .eval() and abandon with .no_grad() below?
        print("Models in training mode...")
...
if __name__ == "__main__":
    ...
    if dataset == 'cifar10':
        train_loader, valid_loader, test_loader = get_cifar10_loaders() #_sub(subset_rate)
